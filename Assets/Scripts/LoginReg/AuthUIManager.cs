...
public class AuthUIManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (FirebaseManager.instance.resetPasswordUI.activeSelf == true && Input.GetKeyDown(KeyCode.Tab))
        {
            FirebaseManager.instance.resetPasswordEmail.Select();
        }
    ...
    public IEnumerator ActivateLoginEmailInputField() //keep an eye on this coroutine content if login/reg breaks
    {
        
        if (FirebaseManager.instance.resetPasswordUI.activeSelf == true)
        {
            FirebaseManager.instance.resetPasswordEmail.ActivateInputField();
        }
        else if (loginInput == 0)
        {
