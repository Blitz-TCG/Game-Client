...
/*                        if (clickImages != null) //unsure what this used to do, doesn't seem needed anymore - leave for now just in case
        ...
        if (doubleClick == false)
        {
            ...
            cardInstance.SetProperties(card.id, card.ergoTokenId, card.ergoTokenAmount, card.cardName, card.cardDescription, card.attack, card.HP, card.gold, card.XP, card.fieldLimit, card.clan, card.levelRequired, card.image.sprite, card.frame.sprite, card.cardClass, card.ability, card.requirements);
        }
